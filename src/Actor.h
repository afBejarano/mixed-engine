...
public:
    ...
    Actor(float nThetaRadianRotation, float nGammaRadianRotation, glm::vec3 nPosition, glm::vec3 nScale,
          std::string nModel, std::string nTexture, Collider* nCollider, Component* parent_):
        Component(nullptr), thetaRadianRotation(nThetaRadianRotation), gammaRadianRotation(nGammaRadianRotation),
        position(nPosition), scale(nScale), model(std::move(nModel)), texture(std::move(nTexture)),
        collider(nCollider) {
    }

    Actor(float nThetaRadianRotation, float nGammaRadianRotation, glm::vec3 nPosition, glm::vec3 nScale,
          std::string nModel, std::string nTexture):
        Component(nullptr), thetaRadianRotation(nThetaRadianRotation), gammaRadianRotation(nGammaRadianRotation),
        position(nPosition), scale(nScale), model(std::move(nModel)), texture(std::move(nTexture)) {
    }
    ...
    std::shared_ptr<ComponentTemplate> GetComponent() {
        for (std::shared_ptr component : components) {
            if (std::shared_ptr<ComponentTemplate> casted_component = std::dynamic_pointer_cast<ComponentTemplate>(component)) {
