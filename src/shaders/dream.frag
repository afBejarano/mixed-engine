...
vec2 dreamWarp(vec2 uv) {
    ...
    float pulse = sin(push.time * pulseSpeed) * pulseAmount;
    ...
    // Add wave distortion
    warped.x += sin(warped.y * 10.0 + push.time * 3.0) * 0.02;
    warped.y += cos(warped.x * 8.0 + push.time * 2.0) * 0.02;
...
vec3 dreamColor(vec3 color) {
    ...
    // Add color pulsing
    float colorPulse = sin(push.time * 3.0) * 0.1 + 0.9;
    color *= colorPulse;
    
    // Add subtle rainbow effect
    float rainbow = sin(push.time * 2.0);
...
vec3 addGlow(vec2 uv, vec3 color) {
    ...
    for(float i = -2.0; i <= 2.0; i += 1.0) {
        for(float j = -2.0; j <= 2.0; j += 1.0) {
            ...
            if(weight > 0.0) {
                glow += texture(screenTexture, uv + offset).rgb * weight;
                totalWeight += weight;
            }
        }
    }
...
float detectEdges(vec2 uv) {
    ...
    for(int i = -1; i <= 1; i++) {
        for(int j = -1; j <= 1; j++) {
            ...
            vec3 neighborColor = texture(screenTexture, uv + offset).rgb;
...
void main() {
    // Apply dream warping
    vec2 warped = dreamWarp(fragTexCoord);
    
    // Sample base color with warped coordinates
    vec3 color = texture(screenTexture, warped).rgb;
